...
class SignupGadgetEditFormater{
	...
	function formatQuestions($value, $id = -1){
		...
		$return .= "<div id=\"question_container\">";  		
		...
		foreach($value as $question){
			...
			// Adds new questions
			$return .= "<script>newQuestion('$questionJS', '$typeJS', $optionsJS, $publicJS, $requiredJS, $id);</script>\n";
			$currentPos++;
		}
		...
		if($currentPos == 0){
			$return .= "<script>newEmptyQuestion()</script>\n";
		}
